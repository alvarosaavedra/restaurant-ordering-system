...
{#if isLoggedIn}
	<div class="min-h-screen subtle-gradient">
		<nav class="glass-effect sticky top-0 z-50 border-b border-gray-200/50" aria-label="Main navigation">
				...
				<div class="flex justify-between h-16">
					...
					<div class="flex items-center gap-4">
						<div class="hidden sm:flex items-center gap-2 bg-primary-50 px-3 py-1.5 rounded-full">
							<div class="w-6 h-6 gradient-bg rounded-full flex items-center justify-center" aria-hidden="true">
						...
						<form action="/logout" method="POST" class="inline">
							<button
								type="submit"
								class="inline-flex items-center gap-1 text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors px-3 py-2 rounded-lg hover:bg-gray-100 min-h-[44px]"
								aria-label="Logout"
							>
		...
		{#if user?.role}
			<nav class="bg-white/50 backdrop-blur-sm border-b border-gray-200/50" aria-label="Section navigation">
				<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div class="flex space-x-1 flex-wrap sm:flex-nowrap" role="menubar">
						<!-- Dashboard -->
						<a 
							href="/" 
						...
						{#if user.role === 'delivery' || user.role === 'admin'}
							<a
								...
								role="menuitem"
								aria-current={isActive('/delivery') ? 'page' : undefined}
							>
								<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
		...
		<main id="main-content" class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8" tabindex="-1">
			{@render children()}
		</main>
